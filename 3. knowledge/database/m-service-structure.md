...
| Table | Purpose | Correct Columns |
|-------|---------|-----------------|
| `m_service_workflow` | Workflow definition | `workflow_id`, `name`, `definition`, `category_ids`, `is_active` |
| `m_service_triggers` | Event triggers | `trigger_id`, `service_id`, `trigger_name`, `trigger_type`, `trigger_config`, `active` |
| `t_ticket` | Ticket header | `ticket_id`, `service_id`, `user_id`, `status_id` |
| `t_ticket_detail` | EAV storage (UI) | `ticket_id`, `label_col`, `cstm_col` |
| `t_ticket_work_data` | EAV storage (Backend) | `ticket_id`, `data_key`, `data_value` |
