...
// const { dbConf, dbQuery, dbTMQuery, dbHots } = require('../config/db'); // loaded by controller if needed, or keep here
const { dbConf, dbQuery, dbTMQuery, dbHots } = require('../config/db');

// const express = require('express'); // Not needed if integrated
// const { API_URL, PORT } = process.env;
const { shippingMailNotificationManual } = require('../service/automation/notification');
const { stuffingWeek } = require('../controller/OnlineOrder/order');
...
const startCLI = (AppInstance) => {
    console.log('🚀 Testing Utils CLI Initialized');

    // Use the passed App instance if needed, or just run CLI
