...
const LoginPage = () => {
    ...
    // resolveApiBase() already ran BEFORE React rendered, so if we got here,
    // the server IS reachable. No need for a second slow Axios ping.
    // If apiResolver failed all IPs, isServerReady() returns false → show maintenance.
    const [serverStatus, setServerStatus] = useState(isServerReady());
    const [loading, setLoading] = React.useState(!isServerReady());

    React.useEffect(() => {
        // Only do a live check if resolver couldn't confirm server health
        if (!isServerReady()) {
            Axios.get(API_URL() + "/auth/ping", { timeout: 5000 })
                .then((res) => {
