...
export const UnifiedFormStructureEditor: React.FC<UnifiedFormStructureEditorProps> = ({
      ...
      case 'specialfunc':
        newItem = {
          ...
          data: {
            ...
            element_type: 'cms_content', // cms_content, cms_header, special_table, diagram, html_block
            cms_page_id: null,
            cms_section: null,
            table_type: null,
            html_content: '',
            diagram_data: null,
            uiCondition: '', // visibility condition
          }
        };
        break;
      case 'textblock':
  ...
  const renderUnifiedItem = (item: FormStructureItem, index: number, dbColumnStart: number, dragHandleProps: any) => {
    if (item.type === 'field') {
      ...
      const specialData = item.data as SpecialElement;

      const elementTypeLabels: Record<string, { label: string; icon: string; color: string }> = {
        'cms_content': { label: 'CMS Content', icon: '📄', color: 'pink' },
        'cms_header': { label: 'CMS Header', icon: '📰', color: 'purple' },
        'special_table': { label: 'Special Table', icon: '📊', color: 'green' },
        'diagram': { label: 'Diagram', icon: '📈', color: 'blue' },
        'html_block': { label: 'HTML Block', icon: '🧩', color: 'orange' },
      };

      const typeInfo = elementTypeLabels[specialData.element_type || 'cms_content'] || elementTypeLabels['cms_content'];

      return (
        <div className={`p-4 bg-${typeInfo.color}-50 border-l-4 border-l-${typeInfo.color}-500 rounded-lg shadow-sm`} style={{ borderLeftColor: typeInfo.color === 'pink' ? '#ec4899' : typeInfo.color === 'purple' ? '#a855f7' : typeInfo.color === 'green' ? '#22c55e' : typeInfo.color === 'blue' ? '#3b82f6' : '#f97316' }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div {...dragHandleProps} className="cursor-move">
                <GripVertical className="w-4 h-4 text-gray-400" />
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-medium text-gray-800">
                    {specialData.title || 'Special Element'}
                  </span>
                ...
                <div className="text-xs text-gray-600">
                  {specialData.element_type === 'cms_content' && specialData.cms_page_id && `CMS Page ID: ${specialData.cms_page_id}`}
                  {specialData.element_type === 'special_table' && specialData.table_type && `Table Type: ${specialData.table_type}`}
                  {!specialData.cms_page_id && !specialData.table_type && 'Click edit to configure'}
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateItem(item.id, { ...specialData, title: e.target.value })}
                ...
                <div>
                  ...
                  <Select
                    value={specialData.element_type || 'cms_content'}
                    onValueChange={(value) => updateItem(item.id, { ...specialData, element_type: value as SpecialElement['element_type'] })}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Select element type" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="cms_content">📄 CMS Content - Display CMS page content</SelectItem>
                      <SelectItem value="cms_header">📰 CMS Header - Display CMS headers/banners</SelectItem>
                      <SelectItem value="special_table">📊 Special Table - Approval/condition tables</SelectItem>
                      <SelectItem value="diagram">📈 Diagram - Workflow diagrams, org charts</SelectItem>
                      <SelectItem value="html_block">🧩 HTML Block - Custom HTML/rich content</SelectItem>
                    </SelectContent>
                  </Select>
                </div>

                {/* CMS Content Options */}
                {(specialData.element_type === 'cms_content' || specialData.element_type === 'cms_header') && (
                  <>
                    <div>
                      <Label>CMS Page ID</Label>
                      <Input
                        type="number"
                        value={specialData.cms_page_id || ''}
                        onChange={(e) => updateItem(item.id, { ...specialData, cms_page_id: parseInt(e.target.value) || null })}
                        placeholder="Enter CMS page ID"
                      />
                    </div>
                    <div>
                      <Label>CMS Section (optional)</Label>
                      <Input
                        value={specialData.cms_section || ''}
                        onChange={(e) => updateItem(item.id, { ...specialData, cms_section: e.target.value })}
                        placeholder="e.g., header, body, footer"
                      />
                    </div>
                  </>
                )}
                ...
                {specialData.element_type === 'special_table' && (
                  <div>
                    <Label>Table Type</Label>
                    <Select
                      value={specialData.table_type || ''}
                      onValueChange={(value) => updateItem(item.id, { ...specialData, table_type: value })}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Select table type" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="approval_matrix">Approval Matrix</SelectItem>
                        <SelectItem value="condition_table">Condition Table</SelectItem>
                        <SelectItem value="checklist">Checklist Table</SelectItem>
                ...
                {/* HTML Block Options */}
                {specialData.element_type === 'html_block' && (
                  <div>
                    <Label>HTML Content</Label>
                    <textarea
                      value={specialData.html_content || ''}
                      onChange={(e) => updateItem(item.id, { ...specialData, html_content: e.target.value })}
                ...
                <div>
                  ...
                  <Input
                    value={specialData.uiCondition || ''}
                    onChange={(e) => updateItem(item.id, { ...specialData, uiCondition: e.target.value })}
