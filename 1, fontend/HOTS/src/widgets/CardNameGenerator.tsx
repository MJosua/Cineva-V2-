...
const CardNameGenerator: React.FC = () => {
    ...
    const handleDownload = () => {
        if (generatedCard) {
            const url = generatedCard.download_url.startsWith('/') ? generatedCard.download_url : `/${generatedCard.download_url}`;
                    ...
                    {generatedCard ? (
                        <div className="w-full space-y-4">
                            <div className="bg-white rounded-lg p-4 shadow-lg">
                                <iframe
                                    src={`${API_URL}${generatedCard.download_url.startsWith('/') ? generatedCard.download_url : `/${generatedCard.download_url}`}`}
