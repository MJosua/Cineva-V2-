...
const renderLayerFunction = `
    const renderLayerForm = (layerBlock, onLayerChange, level) => {
        return (
            <VStack spacing={3} align="stretch" key={layerBlock._id}>
                ...
                {layerBlock.type === 'list' && (
                    <VStack spacing={3} align="stretch">
                        ...
                        <ColorPicker label="Text Color" value={layerBlock.props.textColor} onChange={(v) => onLayerChange({ ...layerBlock, props: { ...layerBlock.props, textColor: v } })} />
                    </VStack>
                )}
    ...
    return (
        <Box>
            ...
            <Reorder.Group axis="y" values={children} onReorder={onReorder}>
                {children.map((c, i) => (
                    <Reorder.Item key={c._id || \`layer-\${i}\`} value={c}>
