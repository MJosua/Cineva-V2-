    ...
    return (
        <Box minH="100vh" bg="gray.100" p={8}>
            ...
            {isLoading ? (
                ...
                <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={6}>
                    {accessibleEvents.map((event) => {
                        // Support both legacy flat arrays and new multi-page page objects
                        const blocksArray = Array.isArray(event.blocks)
                            ? event.blocks
                            : (event.blocks?.pages?.["/"] || []);

                        const heroBlock = blocksArray.find(b => b.type?.toLowerCase().includes('hero'));
                        const title = heroBlock?.props?.title || heroBlock?.data?.title || event.name || "Untitled Event";
                        const blockCount = Array.isArray(event.blocks)
                            ? event.blocks.length
                            : Object.values(event.blocks?.pages || {}).reduce((acc, arr) => acc + (arr?.length || 0), 0);
                            ...
                            >
                                <VStack align="start" spacing={3}>
                                    <Flex w="100%" justify="space-between" align="center">
                                        <Icon as={MdEvent} boxSize={8} color="brand.500" />
            ...
            {admin.role === "superadmin" && (
                <Box mt={8}>
                    <Button colorScheme="brand" size="lg" leftIcon={<MdAdd />} onClick={onOpen}>
                        Create New Event
                    </Button>
                </Box>
            )}
            ...
            <Modal isOpen={isOpen} onClose={onClose} size="lg">
                ...
                <ModalContent>
                    ...
                    <ModalFooter>
                        ...
                        <Button colorScheme="brand" onClick={handleCreateEvent}>
                            Create Event
                        </Button>
                    </ModalFooter>
                </ModalContent>
