...
async function generateCustomTicketID(db, service_id, user_id) {
  ...
  const finalUserId = user_id || 1004; // Robust fallback to current user or system admin
  const user = String(finalUserId).padStart(4, "0");
 ...
 */
  ...
  async create(req, res) {
    try {
      ...
      let { company_id, creator_id, creator_email, form_data, title, parent_ticket_id } = req.body || {};

      // Fallback: If not provided in body, use data from token
      if (!creator_id && req.dataToken) {
        creator_id = req.dataToken.user_id || req.dataToken.id || req.dataToken.userId;
        console.log('👤 [ENGINE] Using creator_id from token:', creator_id);
      }
      if (!company_id && req.dataToken) {
        company_id = req.dataToken.company_id || 1; // Default to 1 if no company_id in token
        console.log('🏢 [ENGINE] Using company_id from token:', company_id);
      }
      if (!creator_email && req.dataToken) {
        creator_email = req.dataToken.email || req.dataToken.user_email;
        if (creator_email) console.log('📧 [ENGINE] Using creator_email from token:', creator_email);
      }

      console.log('🔍 Loaded module =', module.module_key || module.module_name);

      // 🧩 [DYNAMIC TITLE] Derive title from form_data if missing
      if (!title && form_data) {
        const getVal = (f) => (f && typeof f === 'object' ? f.value : f);
        title = getVal(form_data.project_title) ||
          getVal(form_data.project_name) ||
          getVal(form_data.title) ||
          getVal(form_data.name);
      }
        ...
        dbHots.getConnection(async (err, conn) => {
          ...
          try {
            ...
            if (Number(sid) === 23 || module.module_key === 'it_project') {
              ...
              // 🧩 [DYNAMIC CATEGORIZATION] If it's a project, override service_name with category label
              const getVal = (f) => (f && typeof f === 'object' ? f.value : f);
              const projectCat = getVal(form_data.project_category);
              if (projectCat) {
                console.log(`📂 [PROJECT] Overriding service_name with category: ${projectCat}`);
                serviceDisplayName = projectCat;
              }
            }
            ...
            if (Number(sid) === 23 || module.module_key === 'it_project') {
              ...
              if (form_data.project_description) {
                timelineEntries.push([
                  ...
                  'project_initial_request', form_data.project_description, 'text',
                  creatorId, new Date(), new Date(), ticket_id, 0,
                  'manual_update', timelineGroupId, 0, 1, 0, null, company_id || 1
                ]);

                // 📊 [REPORT INTEGRATION] Also store in work_data_report for "suggest report" feature
                await p.query(
                  `INSERT INTO hots.t_ticket_work_data_report 
                   (entity_id, ticket_id, assignment_id, content, created_by, created_at)
                   VALUES (?, ?, 0, ?, ?, NOW())`,
                  [ticket_id, ticket_id, form_data.project_description, creatorId]
                );
              }
              ...
              if (timelineEntries.length > 0) {
                console.log(`📝 [PROJECT] Inserting ${timelineEntries.length} initial timeline entries`);
                await p.query(
              ...
              // Also insert as a structured entity so it shows in the "Work Data" table in AssignmentDetailPage
              const appDataEntityId = `PROJECT_DETAILS`;
              const appDataEntries = [];
              for (const [key, val] of Object.entries(form_data)) {
                if (val && typeof val !== 'object') {
                  appDataEntries.push([
                    ticket_id, 0, sid, 'app_data', appDataEntityId,
                    key, String(val), 'text', creatorId, new Date(), new Date(),
                    ticket_id, 0, 'initial_data', appDataEntityId, 0, 1, 0, null, company_id || 1
                  ]);
                }
              }
              if (appDataEntries.length > 0) {
                await p.query(
                  `INSERT INTO hots.t_ticket_work_data
                   (ticket_id, assignment_id, service_id, data_type, entity_id, field_name, field_value, field_type, created_by, created_at, updated_at, root_ticket_id, ticket_depth, entry_type, timeline_group_id, revision, is_latest, is_hidden, snapshot_meta_json, company_id)
                   VALUES ?`,
                  [appDataEntries]
                );
              }
            ...
            // Also scan all form_data values (including RichText HTML) for temp file URLs
            const tempUrlPattern = /\/files\/hots\/temp\/([^"'\s>)]+)/g;
            Object.values(form_data).forEach(val => {
              if (typeof val === 'string') {
                let match;
                while ((match = tempUrlPattern.exec(val)) !== null) {
                  const filename = match[1];
                  // We'll look up by filename if we don't have the ID
                  foundUploadIds.add({ filename });
                }
              }
            });
            ...
            if (foundUploadIds.size > 0) {
              ...
              for (let item of foundUploadIds) {
                ...
                if (typeof item === 'object' && item.filename) {
                  query += 'filename = ?';
                  queryParams.push(item.filename);
                } else {
                  query += 'upload_id = ?';
                  queryParams.push(item);
                }

                const [tempRows] = await p.query(query, queryParams);
                  ...
                  if (!isImageFile) {
                    ...
                    await p.query(
                      `INSERT INTO t_ticket_file 
                              ...
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                      [
                        entityType, entityId, ticket_id, tempFile.filename,
                        tempFile.original_filename, tempFile.file_path,
                        tempFile.uploaded_by, tempFile.upload_date
                      ]
                    );
                  }
                }
            ...
            const triggerContext = {
              ...
              actor: {
                user_id: creator_id,
                name: req.dataToken?.firstname ? `${req.dataToken.firstname} ${req.dataToken.lastname || ''}`.trim() : 'User'
              },
              formData: form_data,
              moduleKey: module.module_key,
              requesterEmail: creator_email,
              userName: req.dataToken?.firstname ? `${req.dataToken.firstname} ${req.dataToken.lastname || ''}`.trim() : 'User',
              serviceName: module.module_name || module.module_key
            };

            await triggerEngine.runTriggersForEvent(module.module_key, 'on_create', triggerContext);
            await triggerEngine.runTriggersForEvent(module.module_key, 'on_submit', triggerContext);

            // 🧩 Extra: if auto-fulfill, run workflow_complete trigger
            if (approvers.length === 0) {
              await triggerEngine.runTriggersForEvent(module.module_key, 'workflow_complete', { ...triggerContext, status: 1 });
            }

            resolve({ ok: true, success: true, ticket_id, next_approver: firstPending, workflow_steps: approvers.length, auto_fulfilled: approvers.length === 0 });
          } catch (e) {
