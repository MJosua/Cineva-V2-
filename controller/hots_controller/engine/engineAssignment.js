...
module.exports = {
    ...
    getMyAssignments: async (req, res) => {
        try {
            ...
            if (status) {
                filterQuery += ' AND ta.assignment_status = ?';
                params.push(status);
            }

            if (search) {
                filterQuery += ` AND (t.ticket_id LIKE ? OR t.title LIKE ? OR ta.notes LIKE ? OR t.service_name LIKE ?)`;
      ...
      `;
            ...
            const [assignments] = await dbHots.promise().query(query, queryParams);
    ...
    getTimeline: async (req, res) => {
        try {
            ...
            // Fetch attached files for these timeline entries
            if (timelineArray.length > 0) {
                const entityIds = timelineArray.map(t => t.entity_id);
                ...
                // Attach files to respective timeline entries
                const fileMap = {};
                files.forEach(f => {
                    if (!fileMap[f.entity_id]) fileMap[f.entity_id] = [];
                    // Ensure full URL using helper
                    const fullUrl = getAbsoluteUrl(req, f.url);
                    fileMap[f.entity_id].push({ id: f.id, name: f.name, url: fullUrl });
                });

                timelineArray.forEach(t => {
                    t.attached_files = fileMap[t.entity_id] || [];
                });
            }
    ...
    /**
     * POST /engine/assignment/upload
     * Upload an image or attachment for a timeline post
     */
    uploadTimelineFile: async (req, res) => {
        try {
            const user_id = req.dataToken.user_id;
            ...
            for (const file of files) {
                ...
                const url = getAbsoluteUrl(req, filePath);

                uploadedFiles.push({
                    temp_id: result.insertId,
                    url: url,
            ...
            console.error('Error uploading timeline file:', error);
    ...
    addTimelineUpdate: async (req, res) => {
        try {
            const { assignmentId } = req.params;
            const { content, images, temp_file_ids } = req.body;
            const user_id = req.dataToken.user_id;
            ...
            try {
                ...
                if (temp_file_ids && Array.isArray(temp_file_ids) && temp_file_ids.length > 0) {
                    ...
                    for (const tempFile of tempFiles) {
                        ...
                        if (!isImageFile) {
                            ...
                            attachedFiles.push({
                                id: tempFile.upload_id,
                                name: tempFile.original_filename,
                                url: process.env.URL_API + tempFile.file_path
                            });
                        }
                    }
                }
                ...
                console.log(`✅ [TIMELINE] Update ${entityId} added to assignment ${assignmentId} by user ${user_id} with ${attachedFiles.length} files`);
