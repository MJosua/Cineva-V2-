...
// ============================================================
// 🆕 Phase 4: Get all stakeholders for a ticket (for SSE notification)
// Returns array of user_ids who should be notified about ticket activity
// ============================================================
const getTicketStakeholders = async (ticketId, excludeUserId = null) => {
    try {
        const [results] = await dbHots.promise().query(`
            SELECT DISTINCT user_id FROM (
                -- Creator
                SELECT created_by as user_id FROM t_ticket WHERE ticket_id = ?
                UNION
                -- Current assignee (Handled by t_ticket_assignment below)
                -- SELECT assigned_to as user_id FROM t_ticket WHERE ticket_id = ? AND assigned_to IS NOT NULL
                -- UNION
                -- Approvers (all in workflow)
                SELECT approver_id as user_id FROM t_ticket_event WHERE ticket_id = ?
                UNION
                -- Team members (if team-assigned)
                SELECT tm.user_id 
                FROM t_ticket t
                JOIN m_company_team_member tm ON tm.team_id = t.assigned_team
                WHERE t.ticket_id = ?
                UNION
                -- Assignment assignees
                SELECT assigned_user_id as user_id FROM t_ticket_assignment WHERE ticket_id = ? AND assigned_user_id IS NOT NULL
            ) AS stakeholders
            WHERE user_id IS NOT NULL AND user_id != COALESCE(?, 0)
        `, [ticketId, ticketId, ticketId, ticketId, ticketId, excludeUserId]);

        return results.map(r => r.user_id);
    } catch (err) {
        console.error('Error getting ticket stakeholders:', err);
        return [];
    }
};
...
module.exports = {
            ...
            switch (parseInt(service_id)) {
                ...
                case 11: // Pricing Structure
                    try {
                        ...
                        VALUES (
                        ?, ?, ?, ${SERVICE_IDS.PRICING_STRUCTURE}, ${STATUS_IDS.SUBMITTED}, ${SERVICE_IDS.PRICING_STRUCTURE}, now()
                        );
                ...
                case 10: // Data Update
                    try {
                        ...
                        let queryInsertTicket = `
                            ...
                            VALUES (?, ?, ?, 9, ${STATUS_IDS.SUBMITTED}, 9, now());
                ...
                case 6: // SRF Sample category  form
                    ...
                    try {
                        ...
                        let queryInsertTicket = `
                            INSERT INTO t_ticket (ticket_id, created_by, reason, service_id, status_id, assigned_team, creation_date)
                            VALUES (?, ?, ?, ${SERVICE_IDS.SRF}, ${STATUS_IDS.SUBMITTED}, ${SERVICE_IDS.SRF}, now() );
            ...
            switch (parseInt(service_id)) {
                ...
                case 11: // IT tech support
                    ...
                    try {
                        ...
                        // 🆕 SSE: Notify relevant users about approval
                        if (global.sseManager) {
                            try {
                                const approver_name = `${req.dataToken?.firstname || ''} ${req.dataToken?.lastname || ''}`.trim() || 'Approver';
                                const [[ticketInfo]] = await dbHots.promise().query(
                                    'SELECT created_by FROM t_ticket WHERE ticket_id = ?', [ticket_id]
                                );
                                if (ticketInfo?.created_by) {
                                    global.sseManager.emitToUser(ticketInfo.created_by, 'ticket_status_update', {
                                        ticketId: ticket_id,
                                        action: 'approved',
                                        service_id: 11,
                                        ...
                                        message: `${approver_name} approved your Pricing Structure request`
                                    });
                                }
                                // Notify the approver to update their badge (decrement)
                                global.sseManager.emitToUser(user_id, 'badge_update', {
                                    type: 'approval_complete',
                                    action: 'decrement',
                                    ticketId: ticket_id
                                });
                            } catch (sseErr) {
                                console.error('SSE emission error (non-blocking):', sseErr);
                            }
                        }
                            ...
                            try {
                                ...
                                if (ticketInfo?.created_by) {
                                    global.sseManager.emitToUser(ticketInfo.created_by, 'ticket_status_update', {
                                        ...
                                        persist: true,
                                        title: '✅ Request Approved',
                                        message: `${approver_name} approved your IT Support request`
                                    });
                                }
                                // Notify the approver to update their badge (decrement)
                                global.sseManager.emitToUser(user_id, 'badge_update', {
                    ...
                    SELECT
                        JSON_ARRAYAGG(
                            JSON_OBJECT(
                                'approver_id', a.approver_id, 
                                'approval_order', a.approval_order,
                                'approver_name', COALESCE(NULLIF(TRIM(CONCAT(IFNULL(u.firstname, ''), ' ', IFNULL(u.lastname, ''))), ''), 'Superior'),
                                'approval_status', a.approval_status,
                                'cancel_remark', a.remark
                            )
                        )
 ...
 LEFT JOIN m_company_team tm ON
    ...
    getTicketComment: async (req, res) => {
        ...
        if (req.dataToken && req.dataToken.user_id) {
            ...
            if (ticket_id) {
                ...
                let countquery =
                    `
                SELECT COUNT(comment_id) cnt FROM t_ticket_comment WHERE ticket_id = ?
                    ...
                    if (err) {
                        ...
                        dbHots.query(commentQuery, [ticket_order, ticket_id, user_id, comment], (err, results) => {
                            ...
                            if (err) {
                                ...
                                // 🆕 Phase 4: SSE ALWAYS runs - Notify ALL stakeholders
                                if (global.sseManager) {
                                    dbHots.promise().query(
                                        ...
                                        const commenter_name = info?.commenter_name || 'Someone';
                                        const service_name = info?.service_name || 'Ticket';

                                        // 🆕 Get ALL stakeholders (creator, assignee, approvers, team, assignments)
                                        const stakeholders = await getTicketStakeholders(ticket_id, user_id);

                                        // Notify each stakeholder
                                        stakeholders.forEach(userId => {
                                            global.sseManager.emitToUser(userId, 'new_comment', {
                                                ticketId: ticket_id,
                                                commenterId: user_id,
                                                commenter_name: commenter_name,
                                                service_name: service_name,
                                                message: `${commenter_name} commented on ${service_name} ticket`,
                                                timestamp: new Date().toISOString()
                                            }, {
                                                persist: true,
                                                title: '💬 New Comment',
                                                message: `${commenter_name} commented on a ticket you're involved in`
                                            });
                                        });
                                    }).catch(err => {
                                        console.error('SSE comment notification error:', err);
                                    });
                                }
  ...
  LEFT JOIN m_company_team tm ON
    ...
    createTicket: async (req, res) => {
        ...
        try {
          ...
          `, [ticket_id, service_id, user_id, current_step]);
            ...
            if (Array.isArray(workflowSteps) && workflowSteps.length > 0) {
                // 🆕 Launcher Pattern: Approval Notification Email
                const approvalNotifId = await createNotification({
                    user_id: user_id,
                    type: 'system_process',
                    title: '📤 Sending Approval Request Email',
                    message: `Processing: Approval notification for ${service.service_name} (Ticket: ${ticket_id})`,
                    data_payload: {
                        status: 'processing',
                        action: 'approval_email',
                        ticket_id: ticket_id,
                ...
                hotsApproveRequest(false, ticket_id)
                    .then(() => {
                        updateNotification(approvalNotifId, {
                            ...
                            message: `Email sent to approvers for ${service.service_name} (Ticket: ${ticket_id})`,
                            data_payload: {
                                status: 'success',
                                action: 'approval_email',
                                ticket_id: ticket_id,
                    ...
                    .catch((err) => {
                        updateNotification(approvalNotifId, {
                            ...
                            message: `Failed to send email for Ticket: ${ticket_id}. Error: ${err.message}`,
                            data_payload: {
                                status: 'error',
                                action: 'approval_email',
                                ticket_id: ticket_id,
            ...
            if (mailAddress && mailAddress.length > 0) {
                ...
                const recipientEmail = mailAddress[0].email;

                // 🆕 Launcher Pattern: Submission Confirmation Email
                const submitNotifId = await createNotification({
                    user_id: user_id,
                    type: 'system_process',
                    title: '📤 Sending Confirmation Email',
                    message: `Processing: Submission confirmation to ${recipientEmail}`,
                    data_payload: {
                        status: 'processing',
                        action: 'submit_email',
                        ticket_id: ticket_id,
                ...
                hotsSubmitMailer(false, ticket_id, user_name, service.service_name, recipientEmail)
                    .then(() => {
                        updateNotification(submitNotifId, {
                            title: '✅ Confirmation Email Sent',
                            message: `Email sent to ${recipientEmail} for ${service.service_name}`,
                            data_payload: {
                                status: 'success',
                                action: 'submit_email',
                                ticket_id: ticket_id,
                    ...
                    .catch((err) => {
                        updateNotification(submitNotifId, {
                            ...
                            message: `Failed to send to ${recipientEmail}. Error: ${err.message}`,
                            data_payload: {
                                status: 'error',
                                action: 'submit_email',
                                ticket_id: ticket_id,
    ...
    -- 🔥 Priority calculation based on ticket age
    CASE 
...
LIMIT ${limit} OFFSET ${offset};
    ...
    rejectTicket: async (req, res) => {
        ...
        try {
            ...
            // 🆕 SSE: Notify ticket creator about rejection
            if (global.sseManager) {
                const [[ticketInfo]] = await dbHots.promise().query(
                    'SELECT created_by FROM t_ticket WHERE ticket_id = ?', [ticket_id]
                );
    ...
    closeTicket: (req, res) => {
        ...
        dbHots.execute(updateCloseQuery, [ticket_id], (err, result) => {
            ...
            // 🆕 SSE: Notify ticket creator about closure
            if (global.sseManager) {
                dbHots.promise().query('SELECT created_by FROM t_ticket WHERE ticket_id = ?', [ticket_id])
                    .then(([[info]]) => {
                        if (info?.created_by) {
                            global.sseManager.emitToUser(info.created_by, 'ticket_status_update', {
                                ticketId: ticket_id,
                                action: 'closed'
                            });
                        }
    ...
    closeTicketservice: (req, res) => {
        ...
        dbHots.execute(updateCloseQuery, [ticket_id], (err, result) => {
            ...
            // 🆕 SSE: Notify ticket creator about service closure
            if (global.sseManager) {
                dbHots.promise().query('SELECT created_by FROM t_ticket WHERE ticket_id = ?', [ticket_id])
                    .then(([[info]]) => {
                        if (info?.created_by) {
    ...
    approveTicket: async (req, res) => {
        ...
        try {
            ...
            // 🆕 Phase 3: Get actor info for rich notifications
            const [[actorInfo]] = await dbHots.promise().query(
                `SELECT CONCAT(firstname, ' ', lastname) as actor_name FROM user WHERE user_id = ?`, [user_id]
            );
            const actor_name = actorInfo?.actor_name || 'Approver';

            // 🆕 Phase 3: Get ticket & service info for rich notifications
            const [[ticketInfo]] = await dbHots.promise().query(
                `SELECT t.created_by, t.title, s.service_name 
                                                                                                             FROM t_ticket t 
                                                                                                             LEFT JOIN m_service s ON s.service_id = t.service_id 
                                                                                                             WHERE t.ticket_id = ?`, [ticket_id]
            );
            const service_name = ticketInfo?.service_name || 'Service';

            // 🆕 Phase 3: Async email with Launcher Pattern (non-blocking)
            if (global.sseManager) {
                const { createNotification, updateNotification } = require('./notification/notificationController');

                // Create "processing" notification for email
                createNotification({
                    user_id: user_id,
                    type: 'email_sending',
                    title: '📤 Sending Approval Email...',
                    message: `Notifying next approver for ${service_name} ticket`,
                    data_payload: { status: 'processing', action: 'approve', ticket_id, service_name, actor: actor_name, initiated_at: new Date().toISOString() }
                }).then(notifId => {
                    ...
                    hotsApproveRequest(false, ticket_id)
                        .then(() => {
                            updateNotification(notifId, {
                                ...
                                message: `${service_name} approval notification delivered`,
                                data_payload: { status: 'success', action: 'approve', ticket_id, service_name, actor: actor_name, completed_at: new Date().toISOString() }
                            });
                        })
                        .catch(err => {
                            updateNotification(notifId, {
                                type: 'email_sending',
                                title: '❌ Email Failed',
                                message: `Failed to send approval email: ${err.message}`,
                                data_payload: { status: 'error', action: 'approve', ticket_id, service_name, actor: actor_name, error: err.message, failed_at: new Date().toISOString() }
                            });
                        });
                }).catch(err => console.error('Notification creation failed:', err));
            } else {
                // Fallback: synchronous if sseManager not available
                hotsApproveRequest(false, ticket_id).catch(err => console.error('Email failed:', err));
            }
            ...
            // 🆕 Phase 3: SSE with RICH payloads
            if (global.sseManager) {
                const now = new Date().toISOString();

                // 1️⃣ Notify ticket CREATOR about status change
                if (ticketInfo?.created_by) {
                    global.sseManager.emitToUser(ticketInfo.created_by, 'ticket_status_update', {
                        ticketId: ticket_id,
                        action: 'approved',
                        ...
                        timestamp: now,
                        message: !nextStep
                            ? `Your ${service_name} request was fully approved by ${actor_name}`
                            : `${actor_name} approved your ${service_name} request (Level ${level})`
                    }, {
                        persist: true,
                        title: !nextStep ? '🎉 Request Approved!' : '✅ Approval Progress',
                        message: !nextStep
                            ? `Your ${service_name} request was fully approved`
                            : `${actor_name} approved Level ${level}`
                    });
                }

                // 2️⃣ Badge DECREMENT for the CURRENT approver (fixes sidebar count)
                global.sseManager.emitToUser(user_id, 'badge_update', {
                    type: 'approval_complete',
                    action: 'decrement',
                    ticketId: ticket_id
                ...
                // 3️⃣ Notify NEXT approver (if exists)
                if (nextStep) {
                    // Fetch next approver ID
                    const [nextApprovers] = await dbHots.promise().query(
                        `SELECT approver_id FROM t_ticket_event WHERE ticket_id = ? AND approval_order = ? AND approval_status = 0`,
                        [ticket_id, nextStep]
                    );
                    for (const approver of nextApprovers) {
                        global.sseManager.emitToUser(approver.approver_id, 'badge_update', {
                            type: 'approval_inbox',
                            action: 'new_item',
                            ticketId: ticket_id,
                            service_name: service_name,
                            message: `New approval request: ${service_name}`
                        }, { persist: true, title: '📋 New Approval Request', message: `${service_name} request needs your approval` });
                    }
                }
            }
