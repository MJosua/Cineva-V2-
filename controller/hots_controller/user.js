            ...
            if (req.dataToken.user_id) {
                ...
                dbConf.query(query, (err, results) => {
                    if (err) {
                        ...
                        log.eorder.error("Error Get port list", err);
                    } else {
                        res.status(200).send(results);
                        log.eorder.info(`get user Port List for ${company_id} success`);
                    }
            ...
            if (req.dataToken.user_id) {
                ...
                dbConf.query(query, (err, results) => {
                    if (err) {
                        ...
                        res.status(200).send(results);
                        log.eorder.info(`get user Port List for ${req.dataToken.company_id} success`);
                    }
...
`
                ...
                dbConf.query(query, (err, results) => {
                    if (err) {
                        res.status(500).send(err);
                        log.eorder.error("Error get company on ship to party", err);
                    } else {
                        res.status(200).send(results);
                    }
                })
    ...
    findntp: async (req, res) => {
        try {
          ...
          `;
            ...
            log.eorder.info(`get user findntp for ${company_id} list success.`, { notifyTP });

        } catch (error) {
            log.eorder.error("Error in findntp queries:", error);
            res.status(500).send(error);
        }
    },
    ostp: async (req, res) => {
            ...
            `;
                ...
                Promise.all([
                    ...
                    .then(([notifyTP, billTP, checkcompanyname, spcbtp, spcntp]) => {
                        ...
                        log.eorder.info(`get user shiptoparty for ${company_id} list success.`);
                    })
                    .catch((err) => {
                        log.eorder.error("Error in ship to party queries:", err);
                        res.status(500).send(err);
                    });
            ...
            if (req.dataToken.user_id) {
               ...
               `
                ...
                dbConf.query(query, (err, results) => {
                    if (err) {
                        res.status(500).send(err);
                        log.eorder.error("Error get detail profile", err);
                    } else {
                        res.status(200).send(results);
                        log.eorder.info(`get user profile ${userID} success`);
                    }
                })
    ...
    addFeedback: async (req, res) => {
        try {
            ...
            dbConf.query(query, parameter, (err, results) => {
                ...
                if (err) {
                    res.status(500).send(err);
                    log.eorder.error("fail user add feedback:", err);
                } else {
                    res.status(200).send(results);
                    log.eorder.info(`user add feedback success `);
        ...
        try {
            if (req.dataToken.user_id) {
                ...
                dbConf.query(query, (err, results) => {
                    if (err) {
                        res.status(500).send(err);
                        log.eorder.error("Error get user feedback", err);
                    } else {
                        res.status(200).send(results);
                        log.eorder.info(`get user feedback ${userID} success`);
                    }
                })
    ...
    addRequstDataChange: async (req, res) => {
        if (req.dataToken.user_id) {
            ...
            dbConf.query(query, parameter,
                (err, results) => {
                    ...
                    if (err) {
                        ...
                        log.eorder.info(`Add Reqest Data Change: SUCCESS `);
                    }
                }
            )
            ...
            res.status(500).send(err);
            log.eorder.error("UNAUTHORIZED Add Reqest Data Change:", err);
        }
        ...
        try {
            if (req.dataToken.user_id) {
                ...
                dbConf.query(query, (err, results) => {
                    ...
                    if (err) {
                        res.status(500).send(err);
                        log.eorder.error("Error get user Banner ", err);
                    } else {
                        res.status(200).send(results);
                        log.eorder.info(`get user Banner ${req.dataToken.uid} success`);
                    }
    ...
    updateEmail: async (req, res) => {
        try {
            if (req.dataToken.user_id) {
                ...
                log.eorder.info("new email list: ", req.body.emailList);
                let employee_id = req.dataToken.employee_id;
               ...
               `
                ...
                dbConf.query(query, parameter, (err, results) => {
                    if (err) {
                        res.status(500).send(err);
                        log.eorder.error("update user email list", err);
                    } else {
                        res.status(200).send(results);
                        log.eorder.info(`update user email list for ${employee_id} success`);
                    }
                }
                )
            } else {
            ...
            log.eorder.error("update user email list", error);
            res.status(500).send(error);
        }
