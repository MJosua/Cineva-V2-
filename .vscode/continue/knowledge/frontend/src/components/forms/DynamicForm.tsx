    ...
    try {
      ...
      unified.forEach((item) => {
        ...
        if (item.value !== undefined && item.label) {
          const key = item.name || `field_${c}`;
          let finalValue = item.value;

          // Auto-convert number types
          if (item.type === 'number' && finalValue !== '' && finalValue !== null && !isNaN(Number(finalValue))) {
        ...
        if (item.type === "rowgroup" && Array.isArray(item.rows)) {
          // For rowgroups, we might want to keep them as an array or flattened
          // The current backend EAV structure flattens them.
          // Let's use a consistent naming convention: rowgroup_id_index_colname
          item.rows.forEach((row, idx) => {
            Object.keys(row)
              .filter((k) => k.endsWith("Value"))
              .forEach((k) => {
                const colName = k.replace("Value", "");
                // Try to find the actual field name from the schema if possible, 
                // but for now, let's use a constructed key that won't collide
                const key = `${item.id}_${idx}_${colName}`;
                engineEav[key] = {
                  label: colName,
                  value: row[k] ?? "",
                  row_index: idx,
      ...
      // Use base endpoint and rely on service_id in body
