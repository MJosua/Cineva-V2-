 ...
 * Simple HTML -> file document engine
 */
const fs = require('fs');
...
class DocumentEngine {
  ...
  init({ dbQuery, engineLoader }) { this.dbQuery = dbQuery; this.engineLoader = engineLoader; }
  async generateFromHtmlForTicket(moduleKey, ticketId, htmlTemplate, context={}) {
    ...
    if (!htmlPdf) {
      ...
      return { path: htmlFile, note:'html saved (no pdf lib)' };
    }
    const file = { content: html };
    try {
      const buf = await htmlPdf.generatePdf(file, { format:'A4' });
