...
export function useSSE(endpoint, eventHandlers = {}) {
    ...
    useEffect(() => {
        ...
        // Auto-append token for Event Engine (Mock Admin)
        // If endpoint is /api/event-engine/stream, we add token
        let finalUrl = endpoint;
        const isAdminStream = endpoint.includes('event-engine/stream');

        // Check local storage for mocked admin
        const adminStr = localStorage.getItem('event_admin');
        const token = adminStr ? 'mock-admin' : 'guest';

        if (isAdminStream && !endpoint.includes('token=')) {
            finalUrl += `?token=${token}`;
        }

        console.log(`[useSSE] Connecting to ${finalUrl}`);
        ...
        evtSource.onopen = () => {
            console.log(`[useSSE] ✅ Connection OPEN: ${url}`);
        };
