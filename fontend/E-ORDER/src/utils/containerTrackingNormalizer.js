...
export const normalizeContainerTrackingData = (rawData) => {
    ...
    console.log(`📦 Normalizing data from: ${sourceType}`);
    ...
    const normalized = {
        ...
        // Jakarta → Tanjung Pelepas → Port Louis → Cape Town
        orderedLocations,
...
export const calculateProgress = (etd, eta) => {
    ...
    if (isNaN(polDate.getTime()) || isNaN(podDate.getTime())) {
        console.warn("⚠️ Invalid date format for progress calculation");
        return 0;
    }
    ...
    if (totalDuration <= 0) {
        console.warn("⚠️ Invalid duration (pod before pol)");
        return 0;
    }
