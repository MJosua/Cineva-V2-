...
function LandingpageUSA() {
    ...
    const [packetCount, setPacketCount] = useState(false)
    ...
    const comparisonDate = new Date('2025-05-31');
        ...
        reader.onload = () => {
            ...
            Tesseract.recognize(reader.result, "eng")
                .then(({ data: { text } }) => {
                    ...
                    // Extract TRAN ID
                    // const match = text.match(/AID[:#]?\s*([A-Z0-9]+)/i);
                    const match = text.match(/Tran ID#[:#]?\s*(\d+)/i);

                    if (!match) {
                     
                    } else {
                        setIdnumber(`${match[1]}`);
                        console.log("AID:", `${match[1]}`)
                    }
    ...
    const handleInput = () => {
        ...
        if (name.trim() === "") {
            setError(prevErrors => {
                if (!prevErrors.some(error => error.category === 'name')) {
                    return [...prevErrors, { category: 'name', message: "Please fill the Legal Name Form." }];
                }
                return prevErrors;
            });
            toast({
                title: "Error",
                description: "Please fill the Legal Name Form.", // Removed undefined `error.message`
                status: "error",
                duration: 3000,
                isClosable: true,
            });
        ...
        if (phone.trim() === "") {
            ...
            toast({
                title: "Error",
                description: "Please provide a valid phone number.", // Removed undefined `error.message`
                status: "error",
                duration: 3000,
                isClosable: true,
            });
        ...
        if (channel.trim() === "") {
            ...
            toast({
                title: "Error",
                description: "Please provide a valid address.", // Channel is used for full address
                status: "error",
                duration: 3000,
                isClosable: true,
            });
        ...
        if (invoice.trim() === "") {
            ...
            toast({
                title: "Error",
                description: "Please provide a valid Email address.", // Channel is used for full address
                status: "error",
                duration: 3000,
                isClosable: true,
            });
        ...
        if (!file) {
            ...
            toast({
                title: "Error",
                description: "Please upload an invoice photo.", // Channel is used for full address
                status: "error",
                duration: 3000,
                isClosable: true,
            });
        ...
        //             return [...prevErrors, { category: 'idnumber', message: "The data ID is not valid." }];
        //         }
        //         return prevErrors;
        //     });
        //     toast({
        //         title: "Error",
        //         description: "The data ID is not valid.", // Channel is used for full address
        //         status: "error",
        //         duration: 3000,
        //         isClosable: true,
        //     });
        ...
        if (!date) {
            setError(prevErrors => {
                if (!prevErrors.some(error => error.category === 'date')) {
                    return [...prevErrors, { category: 'date', message: "Please agree the Terms n Condition." }];
                }
                return prevErrors;
            });
            toast({
                title: "Error",
                description: "Please check that you are older than 18 years old.", // Channel is used for full address
                status: "error",
                duration: 3000,
                isClosable: true,
            });
            ...
            setError(prevErrors => prevErrors.filter(error => error.category !== 'date'));
        }


        if (!packetCount) {
            setError(prevErrors => {
                if (!prevErrors.some(error => error.category === 'packetCount')) {
                    return [...prevErrors, { category: 'packetCount', message: "Please checklist that you are 18 years of age or older." }];
                }
                return prevErrors;
            });
            toast({
                title: "Error",
                description: "Please agree the Terms n Condition .", // Channel is used for full address
                status: "error",
                duration: 3000,
                isClosable: true,
            });
            ...
            setError(prevErrors => prevErrors.filter(error => error.category !== 'packetCount'));
        }
        ...
        if (!hasError && error.length === 0) {
            ...
            formData.append("Country", "704");
            formData.append("column_1", name);
            formData.append("column_2", idnumber);
            formData.append("column_3", phone);
            formData.append("column_4", channel);//Full Address
            formData.append("column_5", invoice);//Email
            // formData.append("column_6", date); // TNC 18Years old
            // formData.append("column_7", packetCount); // TNC Rules and privacy
