...
import AddMoreContainerSummary from "../../../../components/order/AddMoreContainer/AddMoreContainerSummary";
import { Button } from "bootstrap";
import { useData } from "../../../../components/auth/CheckToken/FetchData/DataContext";

import { calculateContainerCBM } from "../../../../utils/cbmCalculator";
...
function AddMoreContainerTest({ mode }) {
    ...
    const handleCheckout = async () => {
        await getExistingPo();
        // console.log("âœ… Refreshed existing PO:");
        let errors = [];
        ...
        if (!hasError) {
            ...
            if (hasInvalidCBM) {
                toast({
                    title: "Invalid Container Load",
                    description: `One or more containers have CBM above the allowed limit with mixed SKU.`,
                    status: "error",
                    duration: 6000,
                    isClosable: true,
                });
    ...
    const hasInvalidCBM = Object.entries(cbmMap).some(([key, cbm]) => {
        const [oi, ci] = key.split("_").map(Number);
        const container = containerOrders[oi].order.detail.containerList[ci];

        const mixedSKU =
            container.Flavour.filter(f => f.sku !== "-1").length > 1;

        if (!mixedSKU || container.custom) return false;
        ...
        // Use allowedCBM (e.g. 100 for same-load, 66 for standard, 63.3 for special)
        return cbm > allowedCBM;
    });
