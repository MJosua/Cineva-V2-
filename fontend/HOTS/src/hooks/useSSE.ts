...
export function useSSE() {
    ...
    useEffect(() => {
        ...
        eventSource.addEventListener('assignment_update', (e) => {
            ...
            const updaterName = payload.updater_name;
            const serviceName = payload.service_name;

            if (action === 'completed') {
                toast({
                    title: '✅ Assignment Completed',
                    description: payload.message || `An assignment on your ticket has been completed`,
                });
            } else if (action === 'timeline_update') {
                toast({
                    title: '📋 Timeline Update',
                    description: payload.message || `${updaterName || 'Someone'} posted an update on ${serviceName || 'assignment'}`,
                });
            } else if (action === 'task_status_change') {
                toast({
                    title: '📌 Task Update',
                    description: payload.message || `${updaterName || 'Someone'} updated a task`,
                });
            }
        });
