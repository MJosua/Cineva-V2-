  ...
  unifiedItems.forEach((item) => {
    ...
    if (item.type === "rowgroup") {
      ...
      item.rows.forEach((row, idx) => {
        const first = row.firstValue;
        const second = row.secondValue;
        const third = row.thirdValue;
        ...
        // QUANTITY
        const quantityCombined = `${second || ""} ${third || ""}`.trim();

        eav[`qty_${idx}`] = {
          label: "Quantity",
          value: quantityCombined,     // ← HERE IS THE FIX ❗
          type: "rowgroup_item"
        };
