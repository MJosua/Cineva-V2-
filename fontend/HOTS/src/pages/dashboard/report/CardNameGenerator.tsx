    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                // Refresh user data to update button state (show Open/Delete)
        ...
        <div className="card-generator-container grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
                    ...
                    {previewHtml ? (
                        <div className="w-full space-y-6 flex flex-col items-center justify-center">
                            ...
                            {generatedCard ? (
                                ...
                                <div className="text-center text-xs text-slate-500 bg-white/50 py-2 rounded-full">
                                    Preview Mode • Click Generate to Create PDF
                                </div>
                            )}
                        </div>
                    ) : (
            ...
            <Card className="bg-white border-slate-200 shadow-md h-full">
                ...
                <CardContent className="space-y-6 p-6">
                    ...
                    {/* Action Buttons */}
                    {selectedUser && selectedUser.existing_card ? (
                    ...
                    {selectedUser && (
                        <Button
                            onClick={() => {
                                // Open profile in new tab (for testing - uses user_id directly)
                                // In production this should use encrypted ID
                                window.open(`/hots/card/card?employee_id=${selectedUser.user_id}`, '_blank');
                            }}
                            variant="outline"
