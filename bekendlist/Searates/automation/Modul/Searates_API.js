...
function runCheck() {
        ...
        try {
            const [rows] = await dbEOrder.execute(`
                   SELECT 
                    ...
                    AND (
                        (
                            (s.status IS NULL OR s.status = '')
                            AND 
                            (MONTH(r.eta) = MONTH(CURDATE()) OR MONTH(r.etd) = MONTH(CURDATE()))
                            AND
                            (YEAR(r.eta) = YEAR(CURDATE()) AND YEAR(r.etd) = YEAR(CURDATE()))
                            AND
                            r.etd <= NOW()
                        )
                        OR
    ...
    // Find CT numbers that are not finished (ETA time date is further than date now)
    // But only those that are not part of a BL or where BL status is not fulfilled
        ...
        try {
            const [rows] = await dbEOrder.execute(`
                    SELECT
                        ...
                        END as final_tracking_type
                    FROM
                        iod.trs_realization r
                    LEFT JOIN 
                        sea_rates.containers s 
                        ON REPLACE(r.cont_id, '-', '') = s.container_number 
                    LEFT JOIN iod.trs_invoice i 
                        ON r.invoice_id = i.invoice_id
                    LEFT JOIN sea_rates.pol l
                        ON s.shipment_id = l.shipment_id
            ...
            // Extract CT numbers into an array
            return ctNumbers; // Return the array if needed elsewhere
        } catch (error) {
    ...
    async function trackMultipleBLs() {
        ...
        for (let i = 0; i < blNumbers.length; i += batchSize) {
            ...
            if (failedResults.length > 0) {
                ...
                failedResults.forEach(fail => console.error(`- ${fail.cont_id}: ${fail.error}`));
                await Promise.all(
                    failedResults.map(fail => saveTrackingLog(fail.cont_id, '0'))
                );
    ...
    async function trackBLWithRateLimit({ cont_id, so_id, sealine = 'auto', type = 'bl' }, retryCount = 0) {
        ...
        try {
            ...
            if (error.response) {
                ...
                console.error(`🔴 Error tracking ${{ cont_id }} :`, error.response.data);
                console.log("Query", url);

                // Handle rate limiting (status 429)
                if (status === 429) {
                    const remainingRequests = error.response.headers?.['x-ratelimit-remaining'];
                    const resetTime = error.response.headers?.['x-ratelimit-reset'] ?? 10; // Default retry in 10s
                    const waitTime = (resetTime * 1000) || 10000; // Convert seconds to ms
                    ...
                    if (remainingRequests !== undefined) {
                        console.warn(`🛑 Remaining Requests: ${remainingRequests}`);
                    }

                    await new Promise(resolve => setTimeout(resolve, waitTime));

                    if (retryCount < 3) {
                        return trackBLWithRateLimit({ cont_id, so_id, sealine, type }, retryCount + 1);
                    } else {
                        console.error(`🚫 Skipping ${{ cont_id }} after ${retryCount} retries due to rate limits.`);
                    }
                }
            } else {
                console.error(`❌ Request failed for ${{ cont_id }}: ${error.message}`);
            }
        ...
        try {
            ...
            if (error.response) {
                ...
                if (status === 429) {
                    ...
                    if (retryCount < 3) {
                        return trackCTWithRateLimit({ cont_id, so_id, sealine, type }, retryCount + 1);
                    } else {
                        console.error(`🚫 Skipping ${{ cont_id }} after ${retryCount} retries due to rate limits.`);
                    }
                }
    ...
    async function saveTrackingLog(blNo, status) {
        let connection;
        try {
            connection = await dbConf.getConnection();
            const logQuery = `
                INSERT INTO tracking_log (bl_no, status, created_at) 
                VALUES (?, ?, NOW())
            `;
            await connection.execute(logQuery, [blNo, status]);
        } catch (error) {
    ...
    async function saveMetadata(metadata) {
        ...
        try {
            ...
            const metadataQuery = `
                ...
                used, total, remaining, type, 
                unique_total, unique_remaining, unique_used,
                creation_date
                ) 
                VALUES (
                ?, ?, ?, ?,
            ...
            await connection.execute(metadataQuery, [
                metadata.api_calls.used || 0,
                metadata.api_calls.total || 0,
                metadata.api_calls.remaining || 0,
                metadata.type || 'unknown',
                metadata.unique_shipments.total || 0,
                metadata.unique_shipments.remaining || 0,
                metadata.unique_shipments.used || 0,
            ]);
        } catch (error) {
    ...
    async function saveToDatabase(data) {
        ...
        try {
            ...
            for (const record of data) {
                // Save tracking log for successful execution
                await saveTrackingLog(record.cont_id, '1');

                // Save metadata from API response


                if (record.details && record.details.data.metadata) {
                    await saveMetadata(record.details.data.metadata);
                }
    ...
    cron.schedule("0 0 */5 * *", async () => {
        ...
        const blNumbers = await findBLNumber();
