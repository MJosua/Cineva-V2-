...
const { dbConf, dbQuery, dbQueryEOrder } = require("../../config/db");
...
module.exports = {
    ...
    , orderRecieved_GmailMailSender: async (user_id, order_id, mailData) => {
        ...
        */
        ...
        let userData = (await dbQueryEOrder(`
            SELECT su.employee_id, me.email dist_mail, mc.company_name  , group_concat(d.email) iod_mail 
                   FROM  sys_user su 
        ...
        setTimeout(async () => {
            ...
            header data: 
            */
            let orderHeaderData = (await dbQueryEOrder(`SELECT mo.order_id, mo.po_buyer, DATE_FORMAT(mo.po_date ,'%b %d, %Y') po_date, mc2.company_name AS ship_to  , concat(mh.harbour_name, ', ', st.txt) AS port_shipment 
        FROM m_order mo 
